...
class BaseModelTest(TestCase):
    def setUp(self):
        ...
        filename = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../../../data/input/sample_data.txt')
...
class TestNBSCalculations(TestCase):
    ...
    def test_calc_nbs(self):
        ...
        self.assertEqual(calculations.nash_bargaining_solution({"a": self.a, "b": self.b}, denominator), 50)
        self.assertEqual(calculations.nash_bargaining_solution(self.actor_issues, self.denominator),
    ...
    def test_calc_adjusted_nbs(self):
        ...
        self.assertEqual(calculations.adjusted_nbs(self.actor_issues, {}, a1, 100, self.denominator), 100)

        self.assertEqual(calculations.adjusted_nbs(self.actor_issues, {}, a2, 0, self.denominator),
                         Decimal(100) / Decimal(3))

        self.assertEqual(calculations.adjusted_nbs(self.actor_issues, {}, a1, 0, self.denominator),
                         Decimal(200) / Decimal(3))

        self.assertEqual(calculations.adjusted_nbs(self.actor_issues, {"a": 100}, a3, 0, self.denominator),
                         Decimal(200) / Decimal(3))
    ...
    def test_by_absolute_move(self):
        ...
        move = calculations.reverse_move(model.actor_issues[e.j.supply.issue], exchange_ratio=dq, actor=e.j)
        ...
        move_1 = calculations.reverse_move(model.actor_issues[e.i.supply.issue], e.i, dq_1)
        self.assertGreater(move_1, abs(e.i.supply.position - e.j.demand.position))

    def test_sumSaliencePower(self):
        self.assertEqual(calculations.sum_salience_power({"a": self.a, "b": self.b, "c": self.c}), (3))

    def test_new_start_position(self):
        self.assertAlmostEqual(calculations.new_start_position(1, 0, 100), 10, delta=1e-10)
        self.assertAlmostEqual(calculations.new_start_position(1, 100, 0), 90, delta=1e-10)
