...
def test_calc_nbs(test_model) -> None:
    denominator = calculations.calc_nbs_denominator(
        {"a": test_model.a, "b": test_model.b},
    )
    ...
    assert calculations.nash_bargaining_solution(
        test_model.actor_issues,
        test_model.denominator,
    ) == decimal.Decimal(200) / decimal.Decimal(3)


def test_calc_adjusted_nbs(test_model) -> None:
    a1 = "a"
    a2 = "b"
    a3 = "c"
    ...
    assert calculations.adjusted_nbs(
        test_model.actor_issues,
        {},
        a2,
        0,
...
def test_by_absolute_move(sample_model) -> None:
    model = sample_model
    ...
    move = calculations.reverse_move(
        model.actor_issues[e.j.supply.issue],
        exchange_ratio=dq,
        actor=e.j,
    )
    ...
    calculations.by_absolute_move(model.actor_issues[e.j.supply.issue], e.j)
...
def test_sum_salience_power(test_model) -> None:
    assert (
        calculations.sum_salience_power(
            {"a": test_model.a, "b": test_model.b, "c": test_model.c},
        )
        == 3
    )


def test_new_start_position() -> None:
...
def test_reverse_move(sum_salience_power) -> None:
...
def test_sum_salience_power_2() -> None:
    actor_issues = [
